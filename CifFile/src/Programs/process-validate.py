...
jj.close()
try:
    cf = emapsCifFile.ReadCif(tmpfile,scantype="flex")
    os.remove(tmpfile)
except emapsCifFile.StarFile.StarError:
    os.remove(tmpfile)
    import sys
...
# Now do the validation...
    ...
    merged_dics = emapsCifFile.merge_dic(diclist)
    validate_cif.output_header(True,filename,input_dics)
    print(emapsCifFile.validate_report(emapsCifFile.Validate(cf,dic= merged_dics,isdic=is_dic),use_html=True))
