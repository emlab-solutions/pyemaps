...
* __Electron Powder Diffraction__ (__POWDER__) module added to __pyemaps__ package. Powder diffraction data generation methos is embedded in __Crystal__ class as:

```
    generatePowder(...)
    plotPowder(...)            
```
The first methods takes user input of high votage and scattering angle 2ϴ, along with many others to generate the electron powder diffraction in intensity array. _powder.py_ sample code included in the package also demonstates a side by side plot of the intensity for two crystals: Silicon and Diamond. The latter data is with absorption.
